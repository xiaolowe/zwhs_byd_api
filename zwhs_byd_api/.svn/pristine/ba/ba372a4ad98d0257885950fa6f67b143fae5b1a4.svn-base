...
public interface StoreOrderDao extends JpaRepository<StoreOrder, Integer>,
	...
	@Query(value="select sum(so.order_amount) as amount from store_order so where so.store_id=?1 and so.order_status>10 and so.order_status != 50",nativeQuery=true)
